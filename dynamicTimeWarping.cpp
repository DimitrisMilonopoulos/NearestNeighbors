...
double DTWDistance(vector<class Point*> *P, vector<class Point*> *Q){
    ...
    for(int i = 0; i < length; i++)
    {
        ...
        {
            if( i == 0 && j == 0){
                C[i][j] = manhattanDist(P->at(i), Q->at(j));
            }
            else if(j > 0 && i == 0)
            {
                C[i][j] = C[i][j-1] + manhattanDist(P->at(i), Q->at(j));
            }
            else if(i > 0 && j == 0)
            {
                C[i][j] = C[i-1][j] + manhattanDist(P->at(i), Q->at(j));

            }
            else
            {
                minimum = min(min(C[i-1][j], C[i-1][j-1]), C[i][j-1]);
                C[i][j] = minimum + manhattanDist(P->at(i), Q->at(j));
                minimum = 0.0;
            }
        }
    }
